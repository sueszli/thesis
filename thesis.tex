...
The ResNet model self-ensembled on intermediary layers~\cite{fort2024ensemble} not only performs exceptionally well but also represents a promising architectural approach to enhance both the reliability and robustness of a model. Given the recency of the paper and the current lack of follow-up research and reproducibility studies, it stands out as a great candidate for further investigation.
...
% Two hCaptcha challenges were selected for our experiments: a classification challenge and a segmentation challenge. The classification challenge overlays images with simple, predictable patterns like grids of colored geometric shapes (e.g., circles or squares). The classification task being previously solved by Plesner et al.~\cite{plesner2024breaking} with 100\% accuracy on reCAPTCHAv2, we were interested in how well their geometric masks perform. The segmentation challenge work 

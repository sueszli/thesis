...
Firstly, we noticed a gap in research when it comes to human-targeting adversarial attacks by popular CAPTCHA providers like reCAPTCHA and hCaptcha. These CAPTCHAs are widely used across the internet to fend off bots, and their effectiveness is backed by their adoption by major companies. Given their proven track record, we thought it would be interesting to explore geometric masks inspired by hCaptcha as a starting point for our research. This approach leverages existing, effective techniques and applies them in a new context to see how well they hold up.
...
Secondly, the self-ensembling algorithm~\cite{fort2024ensemble} not only performs exceptionally well but also represents a promising architectural approach to enhance both the reliability and robustness of a model. Given the recency of the paper and the current lack of follow-up research and reproducibility studies, it stands out as a great candidate for further investigation.

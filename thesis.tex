...
In the hyperparameter search, the final score of each adversarial example was calculated using the following formula:

$$
\text{Adversarial Accuracy} = \frac{(\text{Acc@1}_{\text{adv}} + \text{Acc@5}_{\text{adv}}) / 2 - (\text{Acc@1} + \text{Acc@5}) / 2 + 1}{2}

\text{Perceptual Quality} = \frac{\text{Cosine Similarity} + \text{PSNR} / 100 + \text{SSIM}}{3}

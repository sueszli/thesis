...
\begin{itemize}
	...
	\item Why do robustness and accuracy trade-off~\cite{zhang2019theoretically}\footnote{Should adversarial examples be misalignments from a converged model of reality, then there should not be a trade-off between robustness and accuracy.}?
...
% https://arxiv.org/pdf/2404.13631v1 --> validate the first assumption, they say they found a way to have robustness and accuracy

% Adversarial examples exploit the fact that neural networks are highly sensitive to small perturbations in input data, which can push an input off this manifold or into regions where the network's decision boundaries behave unpredictably.

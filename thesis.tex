...
\begin{examplebox}
	...
	FGSM is one of the earliest and most widely recognized adversarial attack techniques, introduced by Goodfellow et al.~\cite{goodfellow2014explaining}. Initially introduced in the context of image classification, FGSM has since been adapted to various domains, including natural language processing and reinforcement learning~\cite{korkmaz2023detecting}.\\
...
% continue writing my report in a casual but academic tone.

\begin{figure}
    \centering
    \includegraphics[width=0.2\columnwidth]{assets/latentspace.pdf}
    \caption{This is an example graphic.}
    \label{fig:hi}
\end{figure}


% explain that the this definition can be expanded to any domain but due to the visual nature of the human being and how this concept was initially introduced by szegedy et al. in 2014, we will focus on the visual domain in this work.

% continue writing that most literature says "epsilon should be chosen in a way that's imperceptible to human observers" but that actually as soon as we introduce any kind of human perception into the equation, we're no longer talking about adversarial examples this system is harder to formalize. therefore we want to focus on the concept of "semantics perservation" rather than "imperceptibility" in this work as it makes a lot more sense.
% this is because changing certain qualities (exposure, light, sharpness, ...) these changes while massively changing the input $x$ in the pixel space, should have no effect on the semantics of the image. this is a much more formalizable concept than "imperceptibility" and is also more in line with the original definition of adversarial examples.
...
% explain that there are various domains (cv, nlp, rl, ...) that adversarial examples could be applied to.

% explain ways to categorize adversarial examples: white box vs. black box, targeted vs. untargeted, ... (more categories)

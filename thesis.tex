...
Imperceptible adversarial exampels (assuming they are defined correctly, as discussed below) are a subset of the broader category of semantics preserving~\cite{browne2020semantics} adversarial examples.
...
\begin{enumerate}
	...
	\item \textit{Changing few pixels doesn't necessarily preserve semantics}: The assumption that minimal pixel-space changes ensure semantic preservation is also flawed. This is because humans can detect forged low-$\varepsilon$ adversarial examples with high accuracy in both the visual (85.4\%)~\cite{veerabadran2023subtle} and textual ($\geq$70\%)~\cite{herel2023preserving} domain\footnote{Invertible neural networks can partially mitigate this issue in the visual domain~\cite{chen2023imperceptible}}. Additionally, small $\varepsilon$-bounded adversarial perturbations are found to cause misclassification in time-constrained humans~\cite{elsayed2018adversarial}\footnote{It's debatable whether this is a good argument, since we might not be fitting the human judgemnet function but a converging model of reality, as discussed later.}.
\end{enumerate}

In conclusion, the number of pixels changed is not a reliable indicator of imperceptibility, which itself is just a proxy for semantic preservation.
...
% - https://research.google/blog/introducing-the-unrestricted-adversarial-examples-challenge/ -> semantics, not pixels / perceptibility -> we can use masks
% - https://www.reddit.com/r/MachineLearning/comments/49wrt4/adversarial_images_for_deep_learning/?rdt=48134
% - https://www.reddit.com/r/threebodyproblem/comments/1da3bk8/there_is_no_evidence_humans_cant_be_adversarially/

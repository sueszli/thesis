...
% todo:
% 
% - figure a way for citations to be clustered [1-3] instead of [1,2,3]
% - wait for most current experiments to finish
% - use scp to download stuff from cluster -> yjabary@tikgpu10:/scratch/yjabary/self-ensembling/1-resnet
% - chapter 2:
% 	- split in two chapters
% 	- increase fontsize, legend size, reduce "dimensions" -> no zooming should be necessary --> alternatively just put into appendix, given results aren't that interesting
% 	- make figure descriptions longer, it should be self contained, explain how data is encoded visually, interpret it
% - resnet experiment
% 	- already start writing with just the cifar10 subset you have, it's fine if cifar100, imagenette finish later
% 	- open up a notebook, evaluate against more masks with pretrained models in /tmp folder -> you can also just take 5, and try to tell an interesting story
% - self ensemble experiment
% 	- already start writing with just the cifar10 subset you have, it's fine if cifar100, imagenette finish later
% 
% - use citet / citep where possible
% - move citation boxes to the end of the sentences
...
Finally, tools such as the VulnHuntr~\cite{vulnhuntr} and Big Sleep~\cite{bigsleep2024} models have high potential to be misused for malicious purposes. The latter has been particularly successful in automatically detecting zero-day exploits in SQLite in late 2024~\cite{bigsleep2024}.
...
\chapter{Results: HCaptcha Inspired Geometric Masks}
...
$$
\text{Perceptual Quality} = 0.15 \times \text{Cosine\_Sim} + 0.25 \times \text{PSNR} + 0.35 \times \text{SSIM} + 0.25 \times \text{LPIPS}
$$

$$
\text{Adversarial Accuracy} = \frac{\text{Acc@R}_{\text{adv}} - \text{Acc@R}_{\text{benign}}}{\text{Acc@R}_{\text{benign}}}
$$

$$
\text{Final Score} = -\text{AdvAcc} + \text{Perceptual Quality}
$$
...
\chapter{Results: Self-Ensembled ResNet}

...
\usepackage[margin=0pt, top=25mm, bottom=15mm, left=15mm, right=15mm]{geometry} % drop margin
...
Once the authors shared their codebase, we switched to using their implementation for the subsequent experiments. This ensured consistency and made our results directly comparable to theirs.

We found the author's implementation to contain several improvements over our initial version, such as more variation in nature-based noise, very light FGSM training and random resolution shuffling. These features were not listed in the paper but are recommended and the comments in the code said that we should enable them because they would make the results better.

we tried to keep as many hyperparameters unchanged. we initially differentiated between 3 kinds of special training improvements:

% ```
% enable_noise=comb["training_natural"], -> adding nature-inspired noise, jitter, up and downsampling, blur to the data
% enable_random_shuffle=comb["training_natural"], -> shuffling resolutions in the stack
% enable_adversarial_training=comb["training_natural"], -> very light adversarial training in just a few epochs using FGSM
% '''

we initially ran a bunch of experiments to decied which specific of the 3 had the most impact. for instance we found out that just shuffling without anything else reduced adversarial robustness to attacks and that in every single case the combination of all the 3 was the best.

finding out that they don't mutually have a negative impact on each other (given that this is something that wasn't mentioned in the paper), we exclusively decided to flick all of them on or off for the following experiments. Given that the authors did not do so in their paper, we expect to be able to improve upon their results.

we managed to run an initial evaluation this way on CIFAR10 and CIFAR100 as used in the author's codebase. the author's also used MNIST but we opted for imagenette. however due to the down and upsampling being too aggressive for imagenette, it didn't work and we decided to discard the imagenette experiments for our preliminary evaluation of training strategies. we did however fix these issues in the following experiments.
...
using MINE~\cite{pmlr-v80-belghazi18a} and Rényi's $\alpha$-order matrix-based functional~\cite{6954500} to estimate mutual information.

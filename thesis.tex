...
% this is for the authors that said we don't have a threat model
...
% this is for the authors that said we don't conduct experiments on humans
...
% this is to try to explain self-ensembling and intermediate representations
...
% a) there are universarl adversarial examples 
% b) there are no adversarial examples anymore (because there are no misalignments, just optical illusions that would fool humans too ???)

Should the Platonic Representation Hypothesis hold true, it would mean (WOULD IT THOUGHHHH??????) that by converging to a shared model of reality (not that of the human perception), ultimately, a single adversarial example could fool all models, regardless of their architecture, training data or objective function. Moosavi-Dezfooli et al.\ have demonstrated the existence of a single perturbation on most models for all naturally occurring images, adding weight to this possibility~\cite{moosavi2017universal}.

\section{Counterintuitive Properties \& Mental Models}

% dimples dude

Dimple manifold dude is also kind of talking about universal representations and behavior etc.

Understanding the human-machine vision gap provides a foundation for exploring the mysteries of adversarial examples.

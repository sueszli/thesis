...
\section{Experiments: Geometric masks}

The first series of experiments focused on evaluating the performance of state-of-the-art computer vision models on hCaptcha challenges. We specifically targeted two types of challenges: the classification challenge, which involves overlaying images with simple, predictable patterns like grids of colored geometric shapes (e.g., circles or squares) and the segmentation challenge, which embeds images within a perlin-noise-like pattern, smoothes the edges, adds a slight blur and sometimes incorporates the geometric masks from the classification challenge.
